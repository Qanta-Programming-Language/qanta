...
use super::{
    ...
    types::{QantaInternalParserResult, QantaParserResult},
};

pub struct QantaParser<'a> {
    tokens: &'a Vec<Token>,
    stmts: Vec<Stmt>,
    errors: Vec<String>,
    current: usize,
...
impl<'a> QantaParser<'a> {
    ...
    pub fn parse(&mut self) -> QantaParserResult {
    ...
    fn declaration(&mut self) -> QantaInternalParserResult {
    ...
    fn std_declaration(&mut self) -> QantaInternalParserResult {
    ...
    fn clazz_declaration(&mut self) -> QantaInternalParserResult {
    ...
    fn function(&mut self) -> QantaInternalParserResult {
        let name: Token = self.consume(
            Identifier,
            format!(
                "Expected name. ({}:{})",
    ...
    fn foreach_statement(&mut self) -> QantaInternalParserResult {
    ...
    fn const_declaration(&mut self) -> QantaInternalParserResult {
    ...
    fn let_declaration(&mut self) -> QantaInternalParserResult {
        let name: Token = self.consume(
            Identifier,
            format!(
                "Expected variable name. ({}:{})",
    ...
    fn statement(&mut self) -> QantaInternalParserResult {
        if self.match_token(Echo) {
    ...
    fn break_statement(&mut self) -> QantaInternalParserResult {
    ...
    fn continue_statement(&mut self) -> QantaInternalParserResult {
        if self.loop_nesting == 0 {
            PanicHandler::new(
                Some(self.tokens[self.current].line),
                Some(self.tokens[self.current].column),
    ...
    fn return_statement(&mut self) -> QantaInternalParserResult {
    ...
    fn for_statement(&mut self) -> QantaInternalParserResult {
        self.consume(
            LeftParen,
            format!(
                "Expected '(' after 'for'. ({}:{})",
    ...
    fn while_statement(&mut self) -> QantaInternalParserResult {
        self.consume(
            LeftParen,
            format!(
    ...
    fn elif_statement(&mut self) -> QantaInternalParserResult {
        self.consume(
            LeftParen,
            format!(
                "Expected '(' after 'elif'. ({}:{})",
    ...
    fn if_statement(&mut self) -> QantaInternalParserResult {
        self.consume(
            LeftParen,
            format!(
                "Expected '(' after 'if'. ({}:{})",
    ...
    fn block_statement(&mut self) -> QantaInternalParserResult {
    ...
    fn echo_statement(&mut self) -> QantaInternalParserResult {
    ...
    fn expression_statement(&mut self) -> QantaInternalParserResult {
    ...
    fn synchronize(&mut self) {
        ...
        while !self.is_at_end() {
            ...
            match self.peek().token_type {
                Clazz | Fc | Let | For | If | While | Echo | Return => return,
                _ => (),
            }
