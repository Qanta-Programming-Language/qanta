...
impl QantaInterpreter {
    ...
    pub fn interpret(&mut self, stmts: Vec<&Stmt>) -> QantaResult {
        for stmt in stmts {
            match stmt {
                ...
                Stmt::Block { statements } => {
                    ...
                    let block: QantaResult = self.interpret(statements.iter().collect());
    ...
    fn list(&self, invoke: &[String]) {
        invoke.iter().for_each(|f| match f.as_str() {
            "new" => {
                self.environment
                    .define("new_list", self.build_native_fc("new", List::r#gen));
            }
            "size" => {
                self.environment
