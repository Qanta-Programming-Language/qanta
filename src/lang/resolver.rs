...
use super::{expr::Expr, panic::PanicHandler, stmt::Stmt, tokenizer::Token, types::QantaResult};
...
impl Resolver {
    ...
    fn resolve_internal(&mut self, stmt: &Stmt) -> QantaResult {
        match stmt {
    ...
    fn resolve_block(&mut self, stmt: &Stmt) -> QantaResult {
    ...
    fn resolve_extr_var(&mut self, stmt: &Stmt) -> QantaResult {
    ...
    fn resolve_function(&mut self, stmt: &Stmt, fn_type: FunctionType) -> QantaResult {
    ...
    fn resolve_if_stmt(&mut self, stmt: &Stmt) -> QantaResult {
        if let Stmt::If {
            predicate,
            then,
            elf,
    ...
    fn resolve_function_helper(
        ...
        params.iter().try_for_each(|param| {
            let rs: QantaResult = self.declare(param);
            self.define(param);
    ...
    fn declare(&mut self, name: &Token) -> QantaResult {
    ...
    fn resolve_expr(&mut self, expr: &Expr) -> QantaResult {
        match expr {
    ...
    fn resolve_let(&mut self, expr: &Expr, resolve_id: usize) -> QantaResult {
        match expr {
    ...
    fn resolve_local(&mut self, name: &Token, resolve_id: usize) -> QantaResult {
        if !self.scopes.is_empty() {
    ...
    fn resolve_assign(&mut self, expr: &Expr, rs_id: usize) -> QantaResult {
